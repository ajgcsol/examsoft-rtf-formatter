...
def upload_to_sharepoint_oauth(file_content, filename, site_id, drive_id, access_token, folder_path=""):
    ...
    try:
        ...
        # Build the upload path with folder support - FIXED FOR PATH DUPLICATION
        st.write(f"📤 **Upload Debug (PATH DUPLICATION FIX):**")
        st.write(f"- 📄 **Filename**: `{filename}`")
        st.write(f"- 📁 **Input Folder Path**: `{folder_path if folder_path else '(root)'}`")
        st.write(f"- 🚨 **Known Issue**: Files going to `/Exam Procedures/Exam Procedures/` instead of `/Exam Procedures/`")
        
        if folder_path:
            # Clean up folder path - remove leading/trailing slashes and normalize
            cleaned_path = folder_path.strip().strip('/')
            st.write(f"- 🧹 **Cleaned Path**: `{cleaned_path}`")
            
            # SOLUTION: For Teams sites, try to avoid path duplication
            # Split the path and check for duplication patterns
            path_parts = cleaned_path.split('/')
            st.write(f"- 🔧 **Path Parts**: `{path_parts}`")
            
            # If the first part is "Exam Procedures", try without it since it might be implied in Documents library
            if len(path_parts) >= 1 and path_parts[0] == "Exam Procedures":
                # Skip "Exam Procedures" as it might be duplicated by the API
                sub_path_parts = path_parts[1:]  # Skip "Exam Procedures"
                if sub_path_parts:
                    alt_path = '/'.join(sub_path_parts)
                    st.write(f"- 🔄 **Alternative Path (skipping 'Exam Procedures')**: `{alt_path}`")
                    
                    # Encode alternative path
                    encoded_alt_parts = [quote(part) for part in sub_path_parts if part]
                    encoded_folder_path = '/' + '/'.join(encoded_alt_parts) if encoded_alt_parts else ''
                    
                    st.info("🔧 **Using alternative path to avoid duplication**")
                else:
                    # Just upload to root if only "Exam Procedures" was specified
                    encoded_folder_path = ''
                    st.write(f"- 🎯 **Using root path since only 'Exam Procedures' specified**")
            else:
                # Original logic for other paths
                encoded_parts = [quote(part) for part in path_parts if part]
                encoded_folder_path = '/' + '/'.join(encoded_parts) if encoded_parts else ''
                st.write(f"- 🎯 **Using standard path logic**")
            
            upload_path = f"{encoded_folder_path}/{quote(filename)}" if encoded_folder_path else f"/{quote(filename)}"
        else:
            upload_path = f"/{quote(filename)}"
            encoded_folder_path = ''
            
        st.write(f"- 🎯 **Final Encoded Path**: `{encoded_folder_path if encoded_folder_path else '(root)'}`")
        ...
        st.write(f"- 🎯 **Expected Result**: Avoid `/Exam Procedures/Exam Procedures/` duplication")
        
        st.write("**📡 Uploading file with path duplication fix...**")
        ...
        if response.status_code in [200, 201]:
            ...
            actual_file_url = response_data.get('webUrl', 'URL not available')
            file_size = response_data.get('size', len(file_content) if isinstance(file_content, bytes) else 0)
            
            st.success("🎉 **Upload Successful!**")
            
            # Analysis of where it actually went - PATH DUPLICATION CHECK
            st.write(f"- 🌐 **Actual File URL**: {actual_file_url}")
            
            if "Exam%20Procedures/Exam%20Procedures" in actual_file_url or "Exam Procedures/Exam Procedures" in actual_file_url:
                st.error("🚨 **PATH DUPLICATION STILL DETECTED!**")
                st.write("- The file still went to a nested Exam Procedures folder")
                st.write("- **Next steps**:")
                st.write("  1. Try using a different document library")
                st.write("  2. Access 'Exam Procedures' at the site level instead of Documents library")
                st.write("  3. Check if permissions allow site-level folder access")
                
                # Show current vs expected paths
                st.write("**Path Analysis:**")
                st.write(f"- ❌ **Actual**: Contains duplication")
                st.write(f"- ✅ **Expected**: `https://charlestonlaw.sharepoint.com/sites/acad_affairs/Exam%20Procedures/ExamSoft/Import`")
                
            elif "Exam%20Procedures" in actual_file_url or "Exam Procedures" in actual_file_url:
                st.success("✅ **SUCCESS - No duplication detected!**")
                st.write("- The path fix worked correctly!")
                st.write("- File is in the expected Exam Procedures folder structure")
            else:
                st.info("ℹ️ **File uploaded but not in expected Exam Procedures folder**")
                st.write("- This might indicate the folder doesn't exist or permission issues")
                st.write("- Check SharePoint site structure and permissions")
            
            # Create expandable success details
            with st.expander("📋 View Upload Details", expanded=False):
                st.write(f"**📁 SharePoint Location**: [{format_path_readable(upload_path)}]({actual_file_url})")
                st.write(f"**🌐 File URL**: {actual_file_url}")
            ...
            return True, f"✅ Successfully uploaded **{filename}** with path fix!\n🔗 Direct link: {actual_file_url}"
        else:
...
- Status: ✅ Configuration loaded successfully
            ...
            if st.session_state.access_token is None:
                ...
                # Updated folder path with warning about duplication
                st.write("**⚠️ KNOWN ISSUE**: Files may be duplicated to `/Exam Procedures/Exam Procedures/` instead of `/Exam Procedures/`")
                st.write("**🔧 SOLUTION**: The code below attempts to fix this automatically by skipping the first 'Exam Procedures' part")
                
                folder_path = st.text_input(
                    "Folder Path:",
                    value="Exam Procedures/ExamSoft/Import",
                    help="Enter the folder path. The app will automatically try to fix path duplication by skipping the first 'Exam Procedures' part."
                )
                ...
                if upload_files and sharepoint_site_url:
                    ...
                    with col2:
                        if st.button("🚀 Upload to SharePoint (PATH FIXED)", use_container_width=True):
